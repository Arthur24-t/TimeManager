<template>
    <div class="add-team-container">
        <button class="btn btn-add-team" @click="createTeam">Create a Team</button>
        <input class="add-team-input" v-model="newTeam" />
    </div>
    <div v-for="(team, team_index) in teams">
        <div class="team-collapse">
            <h1 class="team-name">Team <input @change="updateTeamName(team_index, $event.target.value)" :value="team.name"
                    class="team_title" /></h1>
            <svg @click="toggleCollapse(team.name)" :class="{ 'rotation': isCollapsed[team.name] }"
                xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 32 20" fill="none">
                <path
            ...
            <button class="btn btn-update" @click="sendUpdateTeamName(team_index)">Change Team Name</button>
            <button class="btn btn-delete" @click="deleteTeam(team.id)">Delete Team</button>
        </div>

        <div v-if="isCollapsed[team.name]">
            <table>
                <thead>
                    <tr>
                        <th>
...
export default {
    ...
    methods: {
        ...
        createTeam() {
            const regex = /^[A-Za-z\-]+$/;
            if (!regex.test(this.newTeam)) createToast({ title: 'No spaces or special character', description: 'Please respect the correct syntax !' }, { type: 'warning', position: 'bottom-right' });
            else {
                const data = {
                    team: {
                        name: this.newTeam
                ...
                POST(ENDPOINTS.CREATE_TEAM, data, this.token)
                    .then((response) => {
                        createToast({ title: 'Add Success', description: `You have sucessfully added the ${this.newTeam} team` }, { type: 'success', position: 'bottom-right' });
                        this.getTeams()
                        this.newTeam = ''
                    })
                    .catch((error) => {
        ...
        updateTeamName(team_index, value) {
            this.teams[team_index]['name'] = value;
        },
        getTeams(newData) {
            GET(ENDPOINTS.GET_ALL_TEAMS, this.token)
                .then((response) => {
                    ...
                    for (let i = 0; i < this.teams.length; i++) {
                        ...
                        if (newData) this.isCollapsed[this.teams[i].name] = false
        ...
        sendUpdateTeamName(team_id) {
            const data = {
                team: {
                    name: this.teams[team_id].name
                }
            }
            PUT(ENDPOINTS.MODIFY_TEAM_NAME + this.teams[team_id].id, data, this.token)
                .then((response) => {
                    createToast({ title: 'Update Success', description: `You have sucessfully updated the ${data.team.name} team` }, { type: 'success', position: 'bottom-right' });
                })
                .catch((error) => {
        ...
        deleteTeam(team_id) {
            DELETE(ENDPOINTS.DELETE_TEAM + team_id, this.token)
                .then((response) => {
                    createToast({ title: 'Deletion Success', description: `You have sucessfully deleted the team` }, { type: 'success', position: 'bottom-right' });
                    this.getTeams()
                })
                .catch((error) => {
                    createToast({ title: 'Deletion Failed', description: 'Please try again' }, { type: 'danger', position: 'bottom-right' });
