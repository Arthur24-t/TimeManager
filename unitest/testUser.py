...
@pytest.fixture(scope="module")
def get_team(session, login_user):
    user_id, _, _ = login_user
    response = session.get(f"{BASE_URL}/api/users/{user_id}")
    assert response.status_code == 200
    teams = response.json().get('data', []).get('teams', [])
    team = next((team for team in teams if team.get('name') == 'testteam'), None)
    team_id = team.get('id')
    return team_id

def test_add_team_to_user(session, get_team, login_user):
    user_id, _, _ = login_user
    team_id = get_team
...
def test_remove_team_to_user(session, get_team, login_user):
    user_id, _, _ = login_user
    team_id = get_team
    url = f"{BASE_URL}/api/users/{user_id}/teams/{team_id}"
    response = session.delete(url)
